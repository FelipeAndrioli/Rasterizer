...
bool create_window(void) {
    ...
    if (!framebuffer) {
        fprintf(stderr, "Error allocating memory to the framebuffer!\n");
        return false;
    }

    framebuffer_texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
...
void render_framebuffer(void) {
    SDL_UpdateTexture(framebuffer_texture, NULL, framebuffer, 
...
void fix_framerate(void) {
    ...
    int waiting_time = MILLISECS_PER_FRAME - time_elapsed;
    
    if (waiting_time > 0 && waiting_time < MILLISECS_PER_FRAME) {
